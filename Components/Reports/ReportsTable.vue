      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="transaction in reportData.transactions" :key="transaction.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ transaction.customer_name || 'Walk-in' }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">KSh {{ transaction.total.toLocaleString() }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ transaction.payment_method }}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">KSh {{ transaction.vat.toLocaleString() }}</td>
          </tr>
        </tbody>
      </table>
    </div>
