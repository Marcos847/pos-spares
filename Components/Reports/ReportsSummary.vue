      ...
      <div class="space-y-3">
        <div class="flex justify-between">
          ...
          <span class="font-medium"
            >KSh {{ reportData.totalSales?.toLocaleString() || 0 }}</span
          >
        </div>
        <div class="flex justify-between">
          <span class="text-gray-600">Cash Sales:</span>
          <span class="font-medium"
            >KSh {{ reportData.cashSales?.toLocaleString() || 0 }}</span
          >
        </div>
        <div class="flex justify-between">
          <span class="text-gray-600">M-Pesa Sales:</span>
          <span class="font-medium"
            >KSh {{ reportData.mpesaSales?.toLocaleString() || 0 }}</span
          >
        </div>
        <div class="flex justify-between">
          <span class="text-gray-600">Credit Sales:</span>
          <span class="font-medium"
            >KSh {{ reportData.creditSales?.toLocaleString() || 0 }}</span
          >
        </div>
      </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow">
      ...
      <div class="space-y-3">
        ...
        <div class="flex justify-between">
          ...
          <span class="font-medium"
            >KSh {{ avgSaleAmount.toLocaleString() }}</span
          >
        </div>
      </div>
    </div>
...
const lowStockCount = computed(
  () =>
    products.value.filter((p) => p.stock <= p.min_stock && p.stock > 0).length
);
